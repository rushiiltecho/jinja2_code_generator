...
#         {% elif config.auth_type == "oauth2" -%}
#         self.client_id = client_id or os.getenv("{{ config.env_vars.get('client_id', 'CLIENT_ID') }}")
#         self.client_secret = client_secret or os.getenv("{{ config.env_vars.get('client_secret', 'CLIENT_SECRET') }}")
#         self.access_token = access_token
#         {% endif -%}
#         {% for key, env_var in config.env_vars.items() -%}
#         {% if key not in ['client_id', 'client_secret', 'access_token'] -%}
